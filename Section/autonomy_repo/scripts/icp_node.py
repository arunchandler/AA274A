...
from icp_utils import best_fit_transform, icp, open3d_icp
...
class ICPNode(Node):
    ...
    def pcd_callback(self, msg: PointCloud2):
        ...
        pcd_array = get_pcd_array_from_point_cloud(msg)
        translation = self.lidar_pose[:3,3:].T
        transform_points = np.dot(pcd_array, self.lidar_pose[:3, :3].T) + translation
        cloud = o3d.geometry.PointCloud()
        cloud.points = o3d.utility.Vector3dVector(transform_points)
        cloud = cloud.uniform_down_sample(10)
        ...
        if self.use_open3d:
            self.transformation = open3d_icp(cloud, self.prev_pcd, self.transformation)
        else:
            self.transformation = icp(np.asarray(cloud.points), np.asarray(self.prev_pcd.points), self.transformation)
        
        ### TODO: Task 3.7 ###
        
        ### Task 3.7 ###
        self.pose = np.dot(self.pose, self.transformation)
        x = self.pose[0,3]
        y = self.pose[1,3]
