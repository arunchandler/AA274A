...
class Explorer(Node):
    def __init__(self, node_name: str = "explorer") -> None:
        ...
        self.declare_parameter('waiting', False)
        self.spot_time = self.get_clock().now().nanoseconds / 1e9
    ...
    def waiting(self)->bool:
        return self.get_parameter('waiting').value
    
    def stop_callback(self, msg: Bool) -> None:
        if msg.data and not self.previous_reading: 
            self.spot_time = self.get_clock().now().nanoseconds / 1e9
            self.get_logger().info(f"Stop sign seen!")
            self.set_parameters([rclpy.Parameter('waiting', value=True)])
            self.publish_cmd_nav(self.state)
        
        self.previous_reading = msg.data

        if self.get_clock().now().nanoseconds / 1e9 - self.spot_time > 5 and self.waiting:
            self.get_logger().info(f"Activating {self.get_clock().now().nanoseconds / 1e9 - self.last_time}")
            state = self.explore()
            self.publish_cmd_nav(state)
            self.set_parameters([rclpy.Parameter('waiting', value=False)])
    ...
    def send_cmd_nav(self, nav_msg: Bool) -> None:
        ...
        self.get_logger().info(f"Entering Command Navigation")
